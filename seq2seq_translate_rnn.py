...
TGT_VOCAB = ['<BOS>', '<EOS>', 'I', ' ', 'l', 'o', 'v', 'e', 'y', 'o', 'u']  # 英文字符表
...
tgt_seq = [tgt2idx['<BOS>'], tgt2idx['I'], tgt2idx[' '], tgt2idx['l'], tgt2idx['o'], tgt2idx['v'], tgt2idx['e'], tgt2idx[' '], tgt2idx['y'], tgt2idx['o'], tgt2idx['u'], tgt2idx['<EOS>']]
...
def translate(verbose=True):
    ...
    with torch.no_grad():
        ...
        for step in range(20):  # 最多生成20步，防止死循环
            out, hidden = decoder(dec_input, hidden)  # out: [1, 1, vocab], hidden: [1, 1, hidden]
            pred = out.argmax(2)  # [1, 1]
            ch = idx2tgt[pred.item()]
            # 输出当前步信息
            if verbose:
                print(f"Step {step+1}: 预测字符='{ch}', 隐藏状态前5位={hidden[0,0,:5].cpu().numpy()}")
            if ch == '<EOS>':
                break
            result.append(ch)
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        outputs, hidden = self.rnn(x)  # outputs: [batch, seq_len, hidden], hidden: [1, batch, hidden]
...
class Decoder(nn.Module):
    ...
    def forward(self, x, hidden):
        # x: [batch, seq_len]
        x = self.embed(x)  # [batch, seq_len, hidden]
        outputs, hidden = self.rnn(x, hidden)  # outputs: [batch, seq_len, hidden]
...
for epoch in range(300):
    ...
    dec_out, _ = decoder(tgt_tensor, encoder_hidden)      # dec_out: [1, 11, vocab]
